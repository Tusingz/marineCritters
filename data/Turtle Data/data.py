...
with open('Clean_BMC_DB.csv', 'w') as csv_file:
    ...
    with open('new_BMC_database_reordering.csv', 'r') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                if row[22].strip() == "":
                    row[22] = 'No Data'
                csv_writer.writerow([row[0],row[1],row[5],row[6],row[8],row[9],row[10],row[13],row[14],row[15],row[16],row[17],row[18],row[19],row[22],row[23],row[25]])
                line_count += 1
                line_count2 += 1
